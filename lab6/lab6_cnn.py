...
CONV1_CHANNELS = random.choice([16, 32, 64])
CONV2_CHANNELS = random.choice([32, 64, 128])
KERNEL_SIZE = random.choice([3, 5])
POOL_SIZE = 2
FC1_SIZE = random.choice([128, 256, 512])
FC2_SIZE = random.choice([64, 128, 256])
DROPOUT_RATE = random.uniform(0.2, 0.5)
L2_REGULARIZATION = 10 ** random.uniform(-5, -3)
...
print(f"  CONV1_CHANNELS: {CONV1_CHANNELS}")
print(f"  CONV2_CHANNELS: {CONV2_CHANNELS}")
print(f"  KERNEL_SIZE: {KERNEL_SIZE}x{KERNEL_SIZE}")
print(f"  FC1_SIZE: {FC1_SIZE}")
print(f"  FC2_SIZE: {FC2_SIZE}")
print(f"  DROPOUT_RATE: {DROPOUT_RATE:.3f}")
print(f"  L2_REGULARIZATION: {L2_REGULARIZATION:.6f}")

class CNN(nn.Module):
    def __init__(self, conv1_channels=32, conv2_channels=64, kernel_size=3,
                 fc1_size=128, fc2_size=64, dropout_rate=0.25):
        super(CNN, self).__init__()
        
        self.conv1 = nn.Conv2d(1, conv1_channels, kernel_size=kernel_size, padding=kernel_size//2)
        self.bn1 = nn.BatchNorm2d(conv1_channels)
        self.pool1 = nn.MaxPool2d(2, 2)
        
        self.conv2 = nn.Conv2d(conv1_channels, conv2_channels, kernel_size=kernel_size, padding=kernel_size//2)
        self.bn2 = nn.BatchNorm2d(conv2_channels)
        self.pool2 = nn.MaxPool2d(2, 2)
        
        conv_output_size = 7 * 7 * conv2_channels
        
        self.fc1 = nn.Linear(conv_output_size, fc1_size)
        self.bn3 = nn.BatchNorm1d(fc1_size)
        self.dropout1 = nn.Dropout(dropout_rate)
        
        self.fc2 = nn.Linear(fc1_size, fc2_size)
        self.bn4 = nn.BatchNorm1d(fc2_size)
        self.dropout2 = nn.Dropout(dropout_rate)
        
        self.fc_out = nn.Linear(fc2_size, 10)
...
def plot_results_with_validation(train_losses, val_losses, train_accuracies, val_accuracies, test_accuracy):
    ...
    params_text = (
        f"ГИПЕРПАРАМЕТРЫ CNN МОДЕЛИ:\n"
        f"Batch Size: {BATCH_SIZE} | "
        f"Learning Rate: {LEARNING_RATE:.6f} | "
        f"Epochs: {EPOCHS}\n"
        f"Conv Layers: 1→{CONV1_CHANNELS}→{CONV2_CHANNELS} | "
        f"Kernel: {KERNEL_SIZE}x{KERNEL_SIZE} | "
        f"FC Layers: {FC1_SIZE}→{FC2_SIZE}\n"
        f"Dropout: {DROPOUT_RATE:.3f} | "
        f"L2 Reg: {L2_REGULARIZATION:.6f}\n"
    ...
    ax1.set_title('График потерь CNN', fontsize=12, fontweight='bold')
    ...
    ax2.set_title('График точности CNN', fontsize=12, fontweight='bold')
    ax2.legend(fontsize=10)
    ax2.grid(True, alpha=0.3)
    
    fig.suptitle(f'Сверточная нейронная сеть для MNIST - Результаты обучения\n'
...
save_dir = f"mnist_cnn_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
print(f"\nCNN модель сохранена в: {model_path}")
